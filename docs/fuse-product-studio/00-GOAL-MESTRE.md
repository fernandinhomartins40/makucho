...
- Produção após `011607b`: workflow de build/deploy concluído; home e login capturados em 390/820/1280/1440 px. A validação detectou overflow horizontal na home em 1280/1440 px por menu completo e imagem editorial de destaque que é um placeholder azul. O breakpoint do menu foi corrigido para <1600 px, publicado em `be76aaf` e retestado sem overflow nos quatro viewports. Registros de mídia não foram alterados.
- Integração local: PostgreSQL novo migrado e populado; API com storage em disco respondeu ao health check. O fluxo anônimo → login `SUPER_ADMIN` → criar/editar/ler/excluir tag → conferir auditoria passou via HTTP e banco real (`validate-local-api.mjs`). O `.env` local não autentica em produção (uma tentativa 401), portanto o CMS publicado e demais papéis seguem pendentes de validação.
